...
def main():
    ...
    * :ref:`中文API <lif_fc_mnist.main-cn>`
    ...
    # 保存模型
    torch.save(net, model_output_dir + "/SNN_sj/lif_snn_mnist.ckpt")
    # 读取模型
    ...
    with torch.no_grad():
        ...
        np.save("SNN_sj/v_t_array.npy",v_t_array)
        s_t_array = output_layer.s_seq.cpu().numpy().squeeze().T  # s_t_array[i][j]表示神经元i在j时刻释放的脉冲，为0或1
        np.save("SNN_sj/s_t_array.npy",s_t_array)

    train_accs = np.array(train_accs)
    np.save('SNN_sj/train_accs.npy', train_accs)
    test_accs = np.array(test_accs)
    np.save('SNN_sj/test_accs.npy', test_accs)
